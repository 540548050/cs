...
class Wall extends Component{
    ...
    loadList(){
        api.getNewsDetail({type:'0'}).then(res=>{
            console.log(res)
            this.setState({
                dataList:res.verify
            })
    ...
    check(id,news_img,comment){
        this.props.history.push(`/news/auth/detail?id=${id}&checked=0&img=${news_img}&comment=${comment}`)
    }
    update(id,news_img,comment){
        this.props.history.push(`/news/auth/detail?id=${id}&checked=2&img=${news_img}&comment=${comment}`)
    }
    auth(id,news_img,comment){
        this.props.history.push(`/news/auth/detail?id=${id}&checked=1&img=${news_img}&comment=${comment}`)
    }
    getBtns(status,id,news_img,comment){
        let btn ;
        let btn_1 = <div>
                        <Mbtn type='0' callback={()=>{this.check(id,news_img,comment)}}>查看</Mbtn>
                        <Mbtn type='1' callback={()=>{this.clickDel(id)}}>删除</Mbtn>
                        <Mbtn type='3' callback={()=>{this.update(id,news_img)}}>更新</Mbtn>
                    </div>,
            btn_2 = <div>
                        <Mbtn type='0' callback={()=>{this.check(id,news_img,comment)}}>查看</Mbtn>
                        <Mbtn type='1' callback={()=>{this.clickDel(id)}}>删除</Mbtn>
                        <Mbtn type='2' callback={()=>{this.auth(id,news_img,comment)}}>审核</Mbtn>
                    </div>,
            btn_3 = <div>
                        <Mbtn type='0' callback={()=>{this.check(id,news_img,comment)}}>查看</Mbtn>
                        <Mbtn type='1' callback={()=>{this.clickDel(id)}}>删除</Mbtn>
